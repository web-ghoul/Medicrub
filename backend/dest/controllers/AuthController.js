...
exports.CheckEmailExist = exports.DriverLogin = exports.DriverRegister = void 0;
...
/*                                Driver Login                                */
/* -------------------------------------------------------------------------- */
const DriverLogin = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        /* ------------------------------ Check Inputs ------------------------------ */
        ...
        /* ----------------------- Check If User Already Exist ---------------------- */
        const existUser = yield model_1.User
            .findOne({ "email": loginInputs.username.replace(/ /g, "") }).populate('location');
        if (existUser) {
            /* ---------------------------- Validate Password --------------------------- */
            const validPassword = yield (0, PasswordUtiility_1.ValidatePassword)(loginInputs.password, existUser.password, existUser.salt);
            if (validPassword) {
                /* ---------------------------- Generate Token --------------------------- */
                const token = (0, TokenUtility_1.GenerateSignature)({
                    _id: existUser._id,
        ...
        return res.status(400).json({ message: config_1.LOGIN_FAILED_ERROR_MSG });
    }
    catch (_) {
        console.log(_);
        return res.status(400).json({ message: config_1.DEFAULT_ERROR_MSG });
